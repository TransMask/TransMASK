...
class Panda(PyBulletRobot):
    ...
    def _reset_light_kwargs(self):
        if not self.domain_randomize:
            self.light_kwargs = {}
        else:
            self.light_kwargs = {"lightDirection": np.random.uniform(-1, 1, size=3).tolist(),
                                    "lightColor": np.random.uniform(0.8, 1.0, size=3).tolist(),
                                    "lightDistance": np.random.uniform(0.8, 1.5),
                                    "lightAmbientCoeff": np.random.uniform(0.3, 0.7),
                                    "lightDiffuseCoeff": np.random.uniform(0.3, 0.7),
                                    "lightSpecularCoeff": np.random.uniform(0.0, 0.3)}
    
    def get_obs(self) -> np.ndarray:
        self._reset_light_kwargs()
